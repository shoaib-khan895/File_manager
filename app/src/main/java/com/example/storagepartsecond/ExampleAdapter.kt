...
class ExampleAdapter(
    private var exampleList: Array<File>?,
    private var btn_back: ImageView,
    ...
    override fun onBindViewHolder(holder: ExampleViewHolder, position: Int) {
        ...
        if(exampleList!![position].isDirectory)
        ...
        holder.TV_name.text = exampleList?.get(position)?.name
        btn_back.setOnClickListener{


            val rootPath = Environment.getExternalStorageDirectory().toString()

            if(exampleList?.get(position)!!.parent == rootPath){
                Toast.makeText(applicationContext, "Not allowed", Toast.LENGTH_SHORT).show()
            }else
            {

                var pathMy: String? = null
                val temparr = exampleList!![position].parent.split("/")

                val lis  = temparr.subList(0, temparr.size - 1);
                ...
                val myFile = File(pathMy);

                if(myFile.length()>0)
                {

                    val a =   myFile.listFiles()
            ...
            holder.card.setOnClickListener {
                ...
                if(exampleList!![position].isDirectory)
                {
                    val myFile = File(exampleList!![position].path);
                    ...
                    File(exampleList!![position].path).extension

                    val path = Uri.fromFile(File(exampleList!![position].path))
                    val photoURI = FileProvider.getUriForFile(
                        applicationContext,
                        applicationContext.getApplicationContext().getPackageName().toString() + ".provider",
                        File(exampleList!![position].path)
                    )

                    Intent().apply {
                        action = Intent.ACTION_VIEW
                        setDataAndType(path, getMimeTypeFromUri(photoURI))
